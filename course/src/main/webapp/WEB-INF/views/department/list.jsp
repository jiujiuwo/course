...
<%
	...
	//设置左侧浏览状态
	session.setAttribute("nav", "department");
%>
...
<body class="home" onLoad="ShowErrMsg()">
	...
	<div class="DocumentPage">
		...
		<div class="DocumentPageRightArea">
			...
			<div class="CourseContentHeader">系部管理</div>
			...
			<div class="Course-Table">
				...
				<table class="table table-hover table-bordered"
					...
					<tbody>
						...
						<c:forEach var="department" items="${pagedDeparment.result}">
							<tr>
								<th scope="row"><input type="checkbox" value="">
									${(pagedDeparment.currentPageNo-1) * pagedDeparment.pageSize +index}</th>
								<td>${department.name}</td>
								<td>${department.note}</td>
								<td><button type="button" class="btn btn-default btn-xs"
										onclick="onUpdate('${department.id}','${department.name}','${department.note}')">修改...</button>
									<button type="button" class="btn btn-default btn-xs"
										onclick="onDelete('${department.id}','${department.name}')">删除</button></td>
							</tr>
			...
			<div class="modal-content">
				...
				<form class="form-signin"
					...
					<div class="modal-body">
						...
						<label for="inputname" class="sr-only">系部名称</label> <input
							type="text" id="inputname" class="form-control" name="name"
							placeholder="系部名称" required autofocus> <label
							for="inputnote" class="sr-only">系部描述</label> <input type="text"
				...
				<form class="form-signin"
					...
					<div class="modal-body">
						...
						<label for="inputname" class="sr-only">系部名称</label> <input
							...
							type="text" id="inputname" class="form-control" name="name"
							value="" placeholder="系部名称" required autofocus> <label
							for="inputnote" class="sr-only">系部描述</label> <input type="text"
...
<script>
	...
	<script>
		...
		function OnSelectControlChange(sel_obj) {
			...
			var url = "<c:url value='/department'/>"+"/list.html?t_school_id=" + t_school_id;
		...
		function onSearch() {
			...
			if (st != null && st.trim().length > 0) {
				var url ="<c:url value='/department'/>"+ "/select-" + st + ".html";
		...
		function onUpdate(id, name, note) {
			var url = "location='<c:url value="/department/update-"/>" + id + ".html'";
		...
		function onDelete(t_department_id, departmentname) {
			...
			var url = "location='<c:url value="/department/DELETE-"/>"+t_school_id+"-" + t_department_id + ".html'";

			$('#deleteModal').find('.modal-body #deleteinfo').text(departmentname);
