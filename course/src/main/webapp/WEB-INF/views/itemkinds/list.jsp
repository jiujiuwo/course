...
<%
	...
	//设置左侧浏览状态
	session.setAttribute("nav", "itemkinds");
%>
			...
			<div class="Course-Table">
				...
				<table class="table table-hover table-bordered"
					...
					<tbody>
						...
						<c:forEach var="dataitem" items="${pagedItemKinds.result}">
							<tr>
								<th scope="row"><input type="checkbox" value="">
									${(pagedItemKinds.currentPageNo-1) * pagedItemKinds.pageSize +index}</th>
								<td>${dataitem.name}</td>
								<td>${dataitem.note}</td>
			...
			<div class="modal-content">
				...
				<form class="form-signin" action="<c:url value="/${pagedURI}/add.html"/>"
					...
					<div class="modal-body">
						<h4 class="form-signin-heading">请输入新加作业类型名称</h4>
						<label for="inputname" class="sr-only">作业类型名称</label> <input
							type="text" id="inputname" class="form-control" name="name"
							placeholder="作业类型名称" required autofocus> <label
							for="inputnote" class="sr-only">作业类型描述</label> <input type="text"
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">修改作业类型</h4>
				</div>
