...
<%
	...
	//设置左侧浏览状态
	session.setAttribute("nav", "teachingtype");
%>
...
<body class="home" onLoad="ShowErrMsg()">
	...
	<div class="DocumentPage">
		...
		<div class="DocumentPageRightArea">
			...
			<div class="CourseContentHeader">教师授课类型管理</div>
			...
			<div class="Course-Table">
				...
				<table class="table table-hover table-bordered"
					...
					<tbody>
						...
						<c:forEach var="dataitem" items="${pagedTeachingType.result}">
							<tr>
								<th scope="row"><input type="checkbox" value="">
									${(pagedTeachingType.currentPageNo-1) * pagedTeachingType.pageSize +index}</th>
								<td>${dataitem.name}</td>
								<td>${dataitem.note}</td>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">添加教师授课类型</h4>
				</div>
				...
				<form class="form-signin" action="<c:url value="/${pagedURI}/add.html"/>"
					...
					<div class="modal-body">
						<h4 class="form-signin-heading">请输入新加教师授课类型名称</h4>
						<label for="inputname" class="sr-only">教师授课类型名称</label> <input
							type="text" id="inputname" class="form-control" name="name"
							placeholder="教师授课类型名称" required autofocus> <label
							for="inputnote" class="sr-only">教师授课类型描述</label> <input type="text"
