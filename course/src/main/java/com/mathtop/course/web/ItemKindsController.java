...
import com.mathtop.course.service.ItemKindsService;

@Controller
@RequestMapping("/itemkinds")
public class ItemKindsController extends BaseController {
	 ...
	 */
	@Autowired
	private ItemKindsService itemkindsService;

	private final String strPageURI = "itemkinds";
	...
	public ModelAndView add(HttpServletRequest request, UserContactType ct) {
		...
		ItemKinds dbdata = itemkindsService.getByName(ct.getName());
		...
		if (dbdata == null) {
			...
			Integer pageNo = 1;
			Page<ItemKinds> pagedItemKinds = itemkindsService
					.getPage(pageNo, CommonConstant.PAGE_SIZE);

			mav.addObject("pagedItemKinds", pagedItemKinds);
	...
	public ModelAndView select(@PathVariable String ctIdname) {
		...
		if (ctIdname != null && ctIdname.length() > 0) {
			...
			view.addObject("pagedItemKinds", pagedItemKinds);
			view.setViewName(strPageURI+"/list");
	...
	public ModelAndView update(HttpServletRequest request, ExaminationType item) {
		...
		ItemKinds dbitem = itemkindsService.getByName(item
				.getName());
		...
		if (dbitem == null) {
			...
			itemkindsService.update(item.getId(), item.getName(),
					item.getNote());
	...
	@ResponseBody
	public List<ItemKinds> getAllSchoolList(
			@RequestParam(value = "pageNo", required = false) Integer pageNo) {

		pageNo = pageNo == null ? 1 : pageNo;
		Page<ItemKinds> pagedExaminationType = itemkindsService
				.getPage(pageNo, CommonConstant.PAGE_SIZE);
