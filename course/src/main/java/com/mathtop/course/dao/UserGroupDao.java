...
public class UserGroupDao extends BaseDao<UserGroupDao> {
	...
	private final String GET_USER_COUNT_BY_GROUP_ID = "SELECT count(*) FROM t_user_group WHERE t_group_id=?";
	...
	private final String DELETE_USER_GROUP_BY_USER_ID_GROUP_ID = "DELETE FROM t_user_group WHERE t_group_id=? and t_user_id=?";
	...
	public void deleteByGroupIdAndUserId(String t_group_id, String t_user_id) {
		...
		Object params[] = new Object[] { t_group_id, t_user_id };
		int types[] = new int[] { Types.VARCHAR, Types.VARCHAR };
		getJdbcTemplate().update(DELETE_USER_GROUP_BY_USER_ID_GROUP_ID, params, types);
	...
	public List<Group> getGroupByUserId(String t_user_id) {
		...
		getJdbcTemplate().query(GET_GROUP_BY_USER_ID_NOTPAGED, new Object[] { t_user_id }, new RowCallbackHandler() {
	...
	public List<UserGroup> getUserGroupByt_user_id(String t_user_id) {
		...
		getJdbcTemplate().query(GET_USERGROUP_BY_USER_ID_NOTPAGED, new Object[] { t_user_id },
	...
	public List<UserGroup> getUserGroupByGroupId(String t_group_id) {
		...
		getJdbcTemplate().query(GET_USERGROUP_BY_GROUP_ID_NOTPAGED, new Object[] { t_group_id },
				new RowCallbackHandler() {
					...
					public void processRow(ResultSet rs) throws SQLException {
						...
						ug.setUserId(rs.getString("t_user_id"));
						list.add(ug);
						// System.out.println(rs.getString("t_user_id"));
					}
	...
	public List<UserGroupViewData> getUserGroupViewDataByGroupId(String t_group_id) {
		...
		List<UserGroup> paget_user_ids = getUserGroupByGroupId(t_group_id);
		...
		if (paget_user_ids != null) {
			...
			for (UserGroup ug : paget_user_ids) {
				...
				list.add(getUserGroupViewData(ug));
	...
	private List<UserGroup> getUserGroupByGroupId(String t_group_id, int PageBegin, int PageSize) {
		...
		getJdbcTemplate().query(GET_USERGROUP_BY_GROUP_ID, new Object[] { t_group_id, PageBegin * PageSize, PageSize },
				new RowCallbackHandler() {
	...
	private UserGroupViewData getUserGroupViewData(UserGroup ug) {
		UserGroupViewData gpi = new UserGroupViewData();

		// 基本信息
		UserBasicInfo ubi = userbasicinfoDao.getUserBasicInfoByt_user_id(ug.getUserId());
		gpi.setUserbasicinfo(ubi);

		Teacher t = teacherDao.getTeacherByUserId(ug.getUserId());
		...
		if (t != null) {
			...
			Department dept = departmentteacherDao.getDepartmentByTeacherId(t.getId());
			if (dept != null) {
				gpi.setDepartment(dept);
				School s = schooldepartmentDao.getSchoolByDepartmentId(dept.getId());
			...
			// 说明是学生
			Student stu = studentDao.getStudentByUserId(ug.getUserId());
			...
			if (stu != null) {
				...
				Department dept = naturalclassstudentDao.getDepartmentByStudentId(stu.getId());

				gpi.setDepartment(dept);

				School s = naturalclassstudentDao.getSchoolByStudentId(stu.getId());
				gpi.setSchool(s);
	 ...
	 * 
	 */
	private List<UserGroupViewData> PageQuery(String t_group_id, int PageBegin, int PageSize) {

		List<UserGroupViewData> list = new ArrayList<UserGroupViewData>();

		List<UserGroup> paget_user_ids = getUserGroupByGroupId(t_group_id, PageBegin, PageSize);
	...
	public long getCount(String t_group_id) {
		...
		return getJdbcTemplate().queryForObject(GET_USER_COUNT_BY_GROUP_ID, new Object[] { t_group_id },
	...
	public Page<UserGroupViewData> getPageByGroupId(String t_group_id, int pageNo, int pageSize) {
		...
		return new Page<UserGroupViewData>(startIndex, totalCount, pageSize, data);

	}

	long getGroupCountByt_user_id(String t_user_id) {

		return getJdbcTemplate().queryForObject(GET_GROUPID_COUNT_BY_USER_ID, new Object[] { t_user_id },
				new int[] { Types.VARCHAR }, Long.class);
	}
	...
	public Page<Group> getGroupPageByt_user_id(String t_user_id) {
		...
		List<Group> data = getGroupByUserId(t_user_id);

		return new Page<Group>(startIndex, totalCount, (int) totalCount, data);
