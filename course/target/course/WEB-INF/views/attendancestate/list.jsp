...
<%
	...
	//设置左侧浏览状态
	session.setAttribute("nav", "attendancestate");
%>
...
<body>
	...
	<div class="DocumentPage">
		...
		<div class="DocumentPageRightArea">
			...
			<div class="CourseContentHeader">考勤状况管理</div>
			...
			<table>
				<tr>
					...
					<c:if test="${pagedAttendanceState!=null and pagedAttendanceState.totalCount>10}">
			...
			<div class="Course-Table">
				...
				<table class="table table-hover table-bordered" style="margin-bottom: -10px;">
					...
					<tbody>
						...
						<c:forEach var="dataitem" items="${pagedAttendanceState.result}">
							<tr>
								<th scope="row"><input type="checkbox" value="">
									${(pagedAttendanceState.currentPageNo-1) * pagedAttendanceState.pageSize +index}</th>
								<td>${dataitem.name}</td>
								<td>${dataitem.note}</td>
				...
				<mathtop:PageBar pageUrl="/${pagedURI}/list.html" pageAttrKey="pagedAttendanceState" />
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">添加考勤状况</h4>
				</div>

				<form class="form-signin" action="<c:url value="/attendancestate/add.html"/>" method="post">


					<div class="modal-body">
						<h4 class="form-signin-heading">请输入新加考勤状况名称</h4>
						<label for="inputname" class="sr-only">考勤状况名称</label>
						<input type="text" id="inputname" class="form-control" name="name" placeholder="考勤状况名称"
							required autofocus>
						<label for="inputnote" class="sr-only">考勤状况描述</label>
						<input type="text" id="inputnote" class="form-control" name="note" placeholder="考勤状况描述">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">修改考勤状况</h4>
				</div>

				<form class="form-signin" action="<c:url value="/attendancestate/update.html"/>" method="post">


					<div class="modal-body">
						<h4 class="form-signin-heading">请修改考勤状况属性</h4>
						<label for="inputname" class="sr-only">考勤状况名称</label>
						...
						<input type="text" id="inputname" class="form-control" name="name" value=""
							placeholder="考勤状况名称" required autofocus>
						<label for="inputnote" class="sr-only">考勤状况描述</label>
	...
	<script>
		function onSearch() {
			...
			if (st != null && st.trim().length > 0) {
				var url = "attendancestate/select-" + st + ".html";
		...
		function onUpdate(id, name, note) {
			var url = "location='<c:url value="/attendancestate/update-"/>"
					+ id + ".html'";
		...
		function onDelete(id, name) {
			var url = "location='<c:url value="/attendancestate/DELETE-"/>"
					+ id + ".html'";
