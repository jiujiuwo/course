		...
		<div class="DocumentPageRightArea">
			...
			<table>
				<tr>
					...
					<c:if test="${pagedMailBoxSendViewData!=null and pagedMailBoxSendViewData.totalCount>10}">
						<td style="width: 10px;"></td>
						<td><div class="btn-group" role="group" aria-label="...">
								<!-- Split button -->
								<div class="btn-group">
			...
			<div class="Course-Table">
						...
						<div class="row show-grid">
							...
							<div class="col-md-4">
								<c:choose>
									<c:when test="${selectedCourseTeachingClassID!=null}">
										<a
											href="<c:url value="/mailbox/sendcontentwithcourse-${selectedCourseTeachingClassID}-${data.send.id}.html"/>">
											<span class="text-success">${data.send.subject}</span>
										</a>


									</c:when>
									<c:otherwise>
										<a href="<c:url value="/mailbox/sendcontent-${data.send.id}.html"/>">
											<span class="text-success">${data.send.subject}</span>
										</a>
									</c:otherwise>
								</c:choose>
							...
							<div class="col-md-2">
								<fmt:formatDate value="${data.send.senddate}" pattern="yyyy-MM-dd HH:mm:ss" />
				...
				<c:choose>
					<c:when test="${not empty selectedCourseTeachingClassID}">
						<mathtop:PageBar pageUrl="/mailbox/sendlistwithcourse-${selectedCourseTeachingClassID}.html"
							pageAttrKey="pagedMailBoxSendViewData" />
					</c:when>
					<c:otherwise>
						<mathtop:PageBar pageUrl="/mailbox/sendlist.html" pageAttrKey="pagedMailBoxSendViewData" />
					</c:otherwise>
				</c:choose>
