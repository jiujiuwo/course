...
<%
	...
	//设置左侧浏览状态
	session.setAttribute("nav", "attendancemode");
%>
...
<body>
	...
	<div class="DocumentPage">
		...
		<div class="DocumentPageRightArea">
			...
			<div class="CourseContentHeader">考勤签到方式管理</div>
			...
			<table>
				<tr>
					...
					<c:if test="${pagedAttendanceMode!=null and pagedAttendanceMode.totalCount>10}">
						<td style="width: 10px;"></td>
						<td><div class="btn-group" role="group" aria-label="...">
								<!-- Split button -->
								<div class="btn-group">
			...
			<div class="Course-Table">
				...
				<table class="table table-hover table-bordered" style="margin-bottom: -10px;">
					...
					<tbody>
						...
						<c:forEach var="dataitem" items="${pagedAttendanceMode.result}">
							<tr>
								<th scope="row"><input type="checkbox" value="">
									${(pagedAttendanceType.currentPageNo-1) * pagedAttendanceMode.pageSize +index}</th>
								<td>${dataitem.name}</td>
								<td>${dataitem.note}</td>
	...
	<script>
		function onSearch() {
			...
			if (st != null && st.trim().length > 0) {
				var url = "attendancemode/select-" + st + ".html";
