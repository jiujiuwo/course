...
<%
	...
	//设置左侧浏览状态
	session.setAttribute("nav", "coursetype");
%>
...
<body>
	...
	<div class="DocumentPage">
		...
		<div class="DocumentPageRightArea">
			...
			<div class="CourseContentHeader">课程性质管理</div>
			...
			<table>
				<tr>
					...
					<c:if test="${pagedCourseType!=null and pagedCourseType.totalCount>10}">
						<td style="width: 10px;"></td>
						<td><div class="btn-group" role="group" aria-label="...">
								<!-- Split button -->
								<div class="btn-group">
			...
			<div class="Course-Table">
				...
				<table class="table table-hover table-bordered" style="margin-bottom: -10px;">
					...
					<tbody>
						...
						<c:forEach var="dataitem" items="${pagedCourseType.result}">
							<tr>
								<th scope="row"><input type="checkbox" value="">
									${(pagedCourseType.currentPageNo-1) * pagedCourseType.pageSize +index}</th>
								<td>${dataitem.name}</td>
								<td>${dataitem.note}</td>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">添加课程性质</h4>
				</div>



				<form class="form-signin" action="<c:url value="/${pagedURI}/add.html"/>" method="post">


					<div class="modal-body">
						<h4 class="form-signin-heading">请输入新加课程性质名称</h4>
						<label for="inputname" class="sr-only">课程性质名称</label>
						<input type="text" id="inputname" class="form-control" name="name" placeholder="课程性质名称"
							required autofocus>
						<label for="inputnote" class="sr-only">课程性质描述</label>
						<input type="text" id="inputnote" class="form-control" name="note" placeholder="课程性质描述">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">修改课程性质</h4>
				</div>
				...
				<form class="form-signin" action="<c:url value='/${pagedURI}/update.html'/>" method="post">
					...
					<div class="modal-body">
						...
						<input type="text" id="inputname" class="form-control" name="name" value=""
							placeholder="课程性质名称" required autofocus>
						<label for="inputnote" class="sr-only">课程性质描述</label>
	...
	<script>
		...
		function onDelete(id, name) {
			var url = "location='<c:url value="/${pagedURI}/DELETE-"/>" + id
					+ ".html'";
