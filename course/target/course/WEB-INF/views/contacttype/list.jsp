...
<%
	...
	//设置左侧浏览状态
	session.setAttribute("nav", "contacttype");
%>
...
<body>
	...
	<div class="DocumentPage">
		...
		<div class="DocumentPageRightArea">
			...
			<div class="CourseContentHeader">联系类型管理</div>
			...
			<table>
				<tr>
					...
					<c:if test="${pagedContactType!=null and pagedContactType.totalCount>10}">
						<td style="width: 10px;"></td>
						<td><div class="btn-group" role="group" aria-label="...">
								<!-- Split button -->
								<div class="btn-group">
			...
			<div class="Course-Table">
				...
				<table class="table table-hover table-bordered" style="margin-bottom: -10px;">
					...
					<tbody>
						...
						<c:forEach var="dataitem" items="${pagedContactType.result}">
							<tr>
								<th scope="row"><input type="checkbox" value="">
									${(pagedContactType.currentPageNo-1) * pagedContactType.pageSize +index}</th>
								<td>${dataitem.name}</td>
								<td>${dataitem.note}</td>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">添加联系类型</h4>
				</div>

				<form class="form-signin" action="<c:url value="/contacttype/add.html"/>" method="post">


					<div class="modal-body">
						<h4 class="form-signin-heading">请输入新加联系类型名称</h4>
						<label for="inputname" class="sr-only">联系类型名称</label>
						<input type="text" id="inputname" class="form-control" name="name" placeholder="联系类型名称"
							required autofocus>
						<label for="inputnote" class="sr-only">联系类型描述</label>
						<input type="text" id="inputnote" class="form-control" name="note" placeholder="联系类型描述">
	...
	<div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form class="form-signin" action="<c:url value="/contacttype/update.html"/>" method="post">
	...
	<script>
		function onSearch() {
			...
			if (st != null && st.trim().length > 0) {
				var url = "contacttype/select-" + st + ".html";
		...
		function onUpdate(id, name, note) {
			var url = "location='<c:url value="/contacttype/update-"/>" + id
					+ ".html'";
		...
		function onDelete(id, name) {
			var url = "location='<c:url value="/contacttype/DELETE-"/>" + id
					+ ".html'";
